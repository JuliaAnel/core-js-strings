...
function formatTime(minutes, seconds) {
  if (minutes > 10 && seconds > 10) {
    return String(minutes).padEnd(2, ':') + seconds;
  }
  if (minutes < 10 && seconds > 10) {
    return String(minutes).padStart(2, '0') + String(seconds).padStart(3, ':');
  }
  if (minutes > 10 && seconds < 10) {
    return String(minutes).padEnd(2, ':') + String(seconds).padStart(2, '0');
  }
  return String(minutes).padStart(2, '0') + String(seconds).padStart(3, ':0');
}
...
function countVowels(str) {
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (str.charAt(i).match('[aeiouyAEIOUY]')) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  ...
  const newStr = str.toLowerCase().replaceAll(/[^a-z]+/g, '');
